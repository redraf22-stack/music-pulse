...
socket.on('user-video-state',({userId,peerId,userName,enabled,tabId,isAdmin,isMod,isVip})=>{
    ...
    if(enabled){
        ...
        if(!videoStreams[peerId]){ensurePeer().then(()=>socket.emit('request-media',{userId:userId,type:'video'})).catch(()=>{});}
    }else{
...
socket.on('user-screen-state',({userId,peerId,userName,enabled,tabId,isAdmin,isMod,isVip})=>{
    ...
    if(enabled){
        ...
        if(!screenStreams[peerId]){ensurePeer().then(()=>socket.emit('request-media',{userId:userId,type:'screen'})).catch(()=>{});}
    }else{
