...
function togglePlay(){if(myRole!=='admin'&&!isMod)return;socket.emit('update-state',{playing:audio.paused,currentTime:audio.currentTime,startedAt:audio.paused?null:Date.now()+serverTimeOffset});}
...
socket.on('sync',state=>{
    syncPlaying=!!state.playing;if(state.startedAt!==undefined)syncStartedAt=state.startedAt;if(state.currentTime!==undefined)syncCurrentTime=state.currentTime;
