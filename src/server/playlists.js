...
async function resolveTracks(room, withCovers) {
    ...
    const pushLocal = t => { const k = keyOfLocal(t); if (pl.excluded[k] && !pl.selected[k]) return; if (seen.has(k)) return; seen.add(k); out.push({ key: k, owner: host, title: t.title, artist: t.artist, filename: t.filename, preview: '/local-file?p=' + encodeURIComponent(t.filename), isLocal: true, isUrl: false, cover: t.cover || '', duration: Math.floor(t.duration || 30) }); };
    const pushUrl = (t, nick) => { const k = keyOfUrl(t, nick); if (pl.excluded[k] && !pl.selected[k]) return; if (seen.has(k)) return; seen.add(k); out.push({ key: k, owner: nick, title: t.title, artist: t.artist, id: t.id, rawUrl: t.url, autoCover: t.autoCover, coverSaved: t.cover || '', preview: t.preview || ('/url-proxy?url=' + encodeURIComponent(t.url)), isLocal: true, isUrl: true, duration: 0 }); };
    const pushShared = (t, nick) => { const k = 'shared|' + nick + '|' + t.file; if (pl.excluded[k] && !pl.selected[k]) return; if (seen.has(k)) return; seen.add(k); const sh = (room.sharedMusic || {})[nick] || {}; const o = ovr[k] || {}; out.push({ key: k, owner: nick, title: o.title || t.title, artist: o.artist ? { name: o.artist } : (t.artist ? { name: t.artist } : 'Unknown Artist'), preview: '/url-proxy?url=' + encodeURIComponent((sh.base || '') + '/' + encodeURIComponent(t.file)), isLocal: true, isUrl: false, isShared: true, autoCover: !(o.cover || t.cover), coverSaved: o.cover || t.cover || '', duration: 0 }); };
