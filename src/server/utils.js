...
let localTracksCache = null, lastCacheUpdate = 0; const CACHE_TTL = 60000;
async function getLocalTracks(force) {
    ...
    const tracks = await Promise.all(files.map(f => limit(async () => {
        const meta = await readMetadata(path.join(dir, f)); const base = path.parse(f).name;
        ...
        return { filename: f, title, artist, album, duration: meta?.duration || 0, normalizedTitle: normalizeStr(title), normalizedArtist: normalizeStr(artist), normalizedAlbum: normalizeStr(album) };
    })));
