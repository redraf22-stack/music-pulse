module.exports = function (io, R, utils) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join-voice', () => { if (!socket.roomCode) return; const room = rooms[socket.roomCode]; if (!room || !room.voiceEnabled) return; room.voiceStates[socket.id] = { selfMuted: false, selfDeafened: false, forceMuted: false, forceDeafened: false, videoEnabled: false, screenEnabled: false }; socket.to(socket.roomCode).emit('user-joined-voice', { id: socket.id, name: socket.nickname, peerId: socket.peerId }); R.broadcastUsers(socket.roomCode); });
        socket.on('leave-voice', () => { if (!socket.roomCode) return; const room = rooms[socket.roomCode]; const wv = room?.voiceStates[socket.id]?.videoEnabled; const ws = room?.voiceStates[socket.id]?.screenEnabled; if (room) delete room.voiceStates[socket.id]; socket.to(socket.roomCode).emit('user-left-voice', { id: socket.id, peerId: socket.peerId }); if (wv) io.to(socket.roomCode).emit('user-video-state', { userId: socket.id, peerId: socket.peerId, enabled: false }); if (ws) io.to(socket.roomCode).emit('user-screen-state', { userId: socket.id, peerId: socket.peerId, enabled: false }); R.broadcastUsers(socket.roomCode); });
        socket.on('self-voice-state', ({ muted, deafened }) => { if (!socket.roomCode) return; const room = rooms[socket.roomCode]; if (!room || !room.voiceStates[socket.id]) return; room.voiceStates[socket.id].selfMuted = !!muted; room.voiceStates[socket.id].selfDeafened = !!deafened; R.broadcastUsers(socket.roomCode); });
        socket.on('toggle-video', (en, tabId) => { if (!socket.roomCode || !rooms[socket.roomCode]) return; const room = rooms[socket.roomCode]; if (!room.voiceStates[socket.id]) room.voiceStates[socket.id] = { selfMuted: false, selfDeafened: false, forceMuted: false, forceDeafened: false, videoEnabled: false, screenEnabled: false }; room.voiceStates[socket.id].videoEnabled = !!en; socket.tabId = tabId || 'unknown'; io.to(socket.roomCode).emit('user-video-state', { userId: socket.id, peerId: socket.peerId, userName: socket.nickname, enabled: !!en, tabId: tabId || 'unknown', isAdmin: socket.isAdmin, isMod: socket.isMod, isVip: socket.isVip }); R.broadcastUsers(socket.roomCode); });
        socket.on('toggle-screen', (en, tabId) => { if (!socket.roomCode || !rooms[socket.roomCode]) return; const room = rooms[socket.roomCode]; if (!room.voiceStates[socket.id]) room.voiceStates[socket.id] = { selfMuted: false, selfDeafened: false, forceMuted: false, forceDeafened: false, videoEnabled: false, screenEnabled: false }; room.voiceStates[socket.id].screenEnabled = !!en; socket.tabId = tabId || 'unknown'; io.to(socket.roomCode).emit('user-screen-state', { userId: socket.id, peerId: socket.peerId, userName: socket.nickname, enabled: !!en, tabId: tabId || 'unknown', isAdmin: socket.isAdmin, isMod: socket.isMod, isVip: socket.isVip }); R.broadcastUsers(socket.roomCode); });
    ...
    // ✅ текущий трек становится ПЕРВЫМ в очереди ожидания
        ...
        socket.on('update-state', ns => { if ((!socket.isAdmin && !socket.isMod) || !socket.roomCode) return; const room = rooms[socket.roomCode]; if (!room) return; room.state = { ...room.state, ...ns }; io.to(socket.roomCode).emit('sync', { ...room.state, playHistory: room.playHistory || [] }); });
